 ...
 *       ​x−E[x]​∗γ+β
 * 
 * The mean and standard-deviation are calculated over the last D dimensions, where D is 
 * the dimension of `normalized_shape`. For example, if `normalized_shape` is (3, 5) (a 
 * 2-dimensional shape), the mean and standard-deviation are computed over the last 2 
 * dimensions of the input (i.e. input.mean((-2, -1))). γ and β are learnable affine transform 
 * parameters of `normalized_shape` if `elementwise_affine` is `true`. The standard-deviation is 
 * calculated via the biased estimator, equivalent to `torch.var(input, unbiased=False)`.
 * 
 * @note
 * Unlike Batch Normalization and Instance Normalization, which applies scalar scale and bias 
 * for each entire channel/plane with the `affine` option, Layer Normalization applies per-element 
 * scale and bias with `elementwise_affine`.
 * 
 * @variable
 * weight – the learnable weights of the module of shape `normalized_shape` when `elementwise_affine`
 *   is set to `true`. The values are initialized to 1.
 * bias – the learnable bias of the module of shape `normalized_shape` when `elementwise_affine`
 *   is set to `true`. The values are initialized to 0.
 * 
 * @example
 * TODO
 * {{
